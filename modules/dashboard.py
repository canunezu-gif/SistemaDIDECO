...
# USAMOS CTkFrame NORMAL, NO SCROLLABLE PARA EVITAR HUECOS
class DashboardView(ctk.CTkFrame):
    def __init__(self, parent, db):
        super().__init__(parent, fg_color="transparent")
        self.db = db
        self.fig_barras = None
        self.fig_dona = None
        
        # Header ajustado (pady=10)
        ...
        self.charts_frame.grid_rowconfigure(0, weight=1) # Expandir gráficos
    ...
    def cargar_kpis(self):
        ...
        self.card(self.kpi_frame, "Total Ayudas", str(n_ayudas), 0)
        self.card(self.kpi_frame, "Beneficiarios", str(n_beneficiarios), 1)
        self.card(self.kpi_frame, "Inventario", str(n_stock), 2)
        self.card(self.kpi_frame, "Stock Crítico", str(n_criticos), 3, True)

    def card(self, parent, title, value, col, is_alert=False):
        color = "#DC2626" if is_alert else COLOR_SIDEBAR
        card = ctk.CTkFrame(parent, fg_color="white", corner_radius=10, height=100)
        card.pack(side="left", expand=True, fill="x", padx=10)
        card.pack_propagate(False)
        ctk.CTkLabel(card, text=title, font=("Arial", 11, "bold"), text_color="gray").place(x=20, y=15)
        ctk.CTkLabel(card, text=value, font=("Arial", 30, "bold"), text_color=color).place(x=20, y=40)
    ...
    def crear_grafico_entregas(self, parent):
        ...
        try:
            ...
            fechas_ord = sorted(conteo.keys(), key=lambda x: datetime.strptime(x, "%d-%m-%Y") if x else datetime.now())[-7:]
            valores = [conteo[f] for f in fechas_ord]
            if not fechas_ord: fechas_ord, valores = ["Sin Datos"], [0]
        except: fechas_ord, valores = ["Error"], [0]

        self.fig_barras, ax = plt.subplots(figsize=(5, 3), dpi=100)
        self.fig_barras.patch.set_facecolor('white')
        ax.bar(fechas_ord, valores, color="#3B82F6", alpha=0.8)
    ...
    def crear_grafico_tipos(self, parent):
        ...
        try:
            ...
            labels, sizes = (list(c.keys()), list(c.values())) if c else (["Vacio"], [1])
